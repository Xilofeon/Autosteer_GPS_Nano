...
void udpSteerRecv(uint16_t dest_port, uint8_t src_ip[IP_LEN], uint16_t src_port, uint8_t* autoSteerUdpData, uint16_t len)
{
        ...
        if (autoSteerUdpData[0] == 0x80 && autoSteerUdpData[1] == 0x81 && autoSteerUdpData[2] == 0x7F) //Data
        {
            if (autoSteerUdpData[3] == 0xFE)  //254
            {
                ...
                if (useBNO08x)
                {
                    ...
                    {
                        yaw = (bno08x.getYaw()) * CONST_180_DIVIDED_BY_PI; // Convert yaw / heading to degrees
                        yaw = -yaw; //BNO085 counter clockwise data to clockwise data
                        ...
                        {
                          roll = (bno08x.getPitch())* CONST_180_DIVIDED_BY_PI; // Convert pitch to degrees
                          pitch = (bno08x.getRoll()) * CONST_180_DIVIDED_BY_PI; //Convert roll to degrees
                        }
                        else
                        {
                          roll = (bno08x.getRoll()) * CONST_180_DIVIDED_BY_PI; //Convert roll to degrees
                          pitch = (bno08x.getPitch()) * CONST_180_DIVIDED_BY_PI; //Convert roll to degrees
                        }
    
                        bno08xHeading10x = (int16_t)(yaw * 10);
                        bno08xRoll10x = (int16_t)(roll * 10);
                ...
                //Steer Data 2 -------------------------------------------------
                if (steerConfig.ShaftEncoder || steerConfig.PressureSensor || steerConfig.CurrentSensor)
                {
                    if (aog2Count++ > 2)
                    {
                        //Send fromAutosteer2
                        if (steerConfig.ShaftEncoder) sensorReading = pulseCount;
            ...
            else if (autoSteerUdpData[3] == 202)
            {
                ...
                if (autoSteerUdpData[4] == 3 && autoSteerUdpData[5] == 202 && autoSteerUdpData[6] == 202)
                {
                    ...
                    //off to AOG
                    ether.sendUdp(scanReply, sizeof(scanReply), portMy, ipDest, portDestination);
                }
            }
        } //end if 80 81 7F
    }
